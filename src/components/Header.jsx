...
const Header = () => {
  ...
  const adminEmails = ["mueezimran1@gmail.com", "sabaz5@yahoo.com"];

  return (
    <>
      <header className="flex items-center justify-between whitespace-nowrap border-b border-solid border-b-[#bfaedb] bg-[#bfaedb] px-4 md:px-10 py-3 relative">
        <div className="flex items-center gap-4 md:gap-8">
          <Link to="/" className="flex items-center gap-2 md:gap-4 text-[#FFFFFF]">
            <h2 className="text-[#FFFFFF] text-lg font-bold leading-tight tracking-[-0.015em]">Kaantay</h2>
          </Link>

          <div className="hidden md:flex items-center gap-5 lg:gap-9">
            <Link to="/" className="text-[#FFFFFF] text-sm font-medium transition">Home</Link>
            <Link to="/products?category=Bracelets" className="text-[#FFFFFF] text-sm font-medium transition">Bracelets</Link>
            <Link to="/products?category=Pendants" className="text-[#FFFFFF] text-sm font-medium  transition">
Pendants</Link>
            <Link to="/products?category=Kaantay" className="text-[#FFFFFF] text-sm font-medium  transition">Kaantay｡</Link>
            <Link to="/products?category=Rings" className="text-[#FFFFFF] text-sm font-medium  transition">Rings</Link>
            {adminEmails.includes(user?.email) && (
              <Link to="/admin" className="text-[#FFFFFF] text-sm font-medium hover:text-[#0c77f2] transition">Admin</Link>
            )}
          </div>
        </div>
...
#bfaedb] items-center justify-center pl-4 rounded-l-lg border-r-0">
              ...
              <input
                placeholder="Search"
                className="form-input flex w-full min-w-0 flex-1 resize-none overflow-hidden rounded-lg text-[#FFFFFF] focus:outline-0 focus:ring-0 border-none bg-[
#bfaedb] focus:border-none h-full placeholder:text-neutral-500 px-4 rounded-l-none border-l-0 pl-2 text-base font-normal leading-normal"
                value={searchQuery}
...
#bfaedb] text-[#FFFFFF] text-sm font-semibold hover:bg-[#
#bfaedb] transition">Logout</button>
...
#bfaedb] text-white text-sm font-semibold hover:opacity-90 transition"
              >
            ...
            <div className="px-4 py-2 space-y-2">
              <div className="flex items-center gap-2 rounded-lg bg-[#FFE9DD] px-3 py-2">
                ...
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && handleSearchSubmit()}
                />
                <button onClick={handleSearchSubmit} className="text-[#FFFFFF] hover:text-[#0c77f2]" aria-label="Search">
                  🔍
                </button>
              </div>
                ...
                {user ? (
                  <>
                    <div className="flex items-center px-4 py-3 space-x-3">
                      ...
                      <div>
                        <p className="font-medium text-[#FFFFFF]">{user.displayName || user.email}</p>
                      </div>
                    </div>
